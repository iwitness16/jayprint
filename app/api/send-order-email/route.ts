...
export async function POST(req: NextRequest) {
  try {
    // sendBeacon may send content-type as text/plain even with a JSON blob,
    // so parse the body regardless of content-type header.
    ...
    console.log('[email-route] received order:', body?.orderId ?? '(no orderId)')
    ...
    console.log('[email-route] SMTP verified OK')
    ...
    await transporter.sendMail({
      from: `"JayCards Orders" <${process.env.MAIL_FROM}>`,
      to: process.env.MAIL_TO,
      subject: `🆕 New Order: ${product} — ${orderId}`,
      html,
    })

    console.log('[email-route] email sent successfully to', process.env.MAIL_TO)
